...
print("[DEBUG] Theta best train is: ", theta_best.flatten())

###################################################################################################################
# TODO: calculate error
"""
error (MSE) = 1/m * SUM (from i=1 to m) (y_predicted_i - y_real)^2
...
#mse = mse / len(x_test)
print(f'MSE test error is: {mse}, b, a (for y = b + ax, from theta) is {theta_best.flatten()}')
...
for i in range(iterations):
    ...
    if i % 5 == 0: 
        print(f'\t[DEBUG] Iteration {i}, gradient_MSE: {gradient_MSE.flatten()}')
        y_standarized_pred = theta[0] + theta[1]*standarized_x_train
        temp_MSE = np.mean((y_standarized_pred - standarized_y_train) ** 2)
        print(f'\t\t Current MSE (for training data): {temp_MSE}')

theta_best = theta
print(f'[DEBUG] gradient_MSE: {gradient_MSE.flatten()}')
print(f'using batch Gradient Descent theta: {theta.flatten()}')

###################################################################################################################
# TODO: calculate error
"""
error (MSE) = 1/m * SUM (from i=1 to m) (y_predicted_i - y_real_i)^2
where m = number of observations (number of rows in x_train / x_test)
y_predicted_i = theta_best[0] + theta_best[1] * x_i

"""
# m = len(x_train)
y_pred_standardized = theta[0] + theta[1]*standarized_x_test
y_pred_unstandardized = y_pred_standardized * y_train_std + y_train_mean

mse = np.mean( (y_pred_unstandardized - y_test) ** 2 )
print(f'(using Batch Gradient Descend) MSE error is: {mse}, b, a (for y = b + ax, from theta) is {theta.flatten()}')

###################################################################################################################
# plot the regression line
x = np.linspace(min(x_test), max(x_test), 100)
x_stand = (x - x_train_mean) / x_train_std
y_stand = float(theta_best[0]) + float(theta_best[1]) * x_stand
y = y_stand * y_train_std + y_train_mean
plt.plot(x, y)
plt.scatter(x_test, y_test)
plt.xlabel('Weight')
plt.ylabel('MPG')
